...
class SubLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        tanh_1_output = F.tanh(self.norm_1(torch.add(x, attn_output)))
        ff_1_output = self.ff_1(tanh_1_output)
        ff_2_output = self.ff_2(F.relu(ff_1_output))
        tanh_2_output = F.tanh(self.norm_2(torch.add(tanh_1_output, ff_2_output)))
...
class Encoder(nn.Module):
    ...
    def forward(self, x):        
        ...
        static_x = self.feature_normalize(static_x)
        ...
        combined_x = self.linear_layer(torch.cat((static_x, dynamic_x), dim=1))
        fushion_x = self.fusion_layer(combined_x, combined_x, combined_x)[0]
